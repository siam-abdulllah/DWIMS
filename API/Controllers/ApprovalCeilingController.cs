...
namespace API.Controllers
{
    public class ApprovalCeilingController : BaseApiController
    {
        ...
        [HttpGet("getBudgetCeilingForRapidPMD/{empID}/{sbu}/{DonationId}")]
        
        public ActionResult<IReadOnlyList<BudgetCeiling>> GetBudgetCeilingForRapidPMD(int empID, string sbu, string DonationId)
        {
            try
            {
                List<SqlParameter> parms = new List<SqlParameter>
                ...
                var results = _dbContext.BudgetCeiling.FromSqlRaw<BudgetCeiling>("EXECUTE SP_BudgetCeilingSearchNewRapidPMD @SBU,@EID,@DID", parms.ToArray()).ToList();
                return results;
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<IReadOnlyList<BudgetCeiling>> GetBudgetCeilingForRapidSales(int empID, string sbu, string DonationId)
        {
            try
            {
                ...
                var results = _dbContext.BudgetCeiling.FromSqlRaw<BudgetCeiling>("EXECUTE SP_BudgetCeilingSearchNewRapidSales @SBU,@EID,@DID", parms.ToArray()).ToList();
                return results;
            }
            catch (Exception ex)
            {
